<template>
	<view>
		<view class="uni-flex">
			...
			<view class="uni-flex-item uni-right" style="font-size:32upx;color:#707070" v-if="showMore" @tap="goReviews">
				<text>More</text>
			...
			<view class="uni-comment" v-for="(item, index) in list" :key="index">
				<view class="uni-comment-list">
					...
					<view class="uni-comment-body">
						...
						<view>
        					<uni-icons type="star-filled" color="#ffdc64" size="26" v-for="i in (item.stars-1+1)" :key="i"/>
							<uni-icons type="star-filled" color="#ccc" size="26" v-for="i in (5-item.stars)" :key="i"/>
						</view>
...
export default {
	...
	methods:{
		goReviews(){
			uni.navigateTo({
				url: '/pages/common/reviews/reviews'
			})
		}
	}
}
